...
s_file_options	validate_map(char *file_name)
{
	char			*buffer;
	s_file_options	options;
	char			chars[3];
	int				c_lines;
	int				intValue;
	char			*buff;
	...
	options.y_x[0] = intValue;
	options.y_x[1] = validate_characters_cols(buff, chars);
	options.chars = chars;
	options.map = create_map(buff, options.y_x);
	return (options);
}
